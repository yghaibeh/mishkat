...
export async function addExpense(db: Db, input: {
  mosqueId: string; category?: string; amount: number; spentBy?: string; note?: string
}, actorUserId?: string) {
  const id = crypto.randomUUID()
  await db.insert(expenses).values({
    id, mosqueId: input.mosqueId, category: input.category ?? null, amount: input.amount,
    spentBy: input.spentBy ?? null, note: input.note ?? null, at: Date.now(),
  }).run()
  await writeAudit(db, { actorUserId: actorUserId ?? null, action: 'add_expense', entity: 'expense', entityId: id, after: { mosqueId: input.mosqueId, amount: input.amount } })
  return { id }
}
