...
export async function registerParticipant(db: Db, input: { competitionId: string; personId: string; mosqueId: string; age: number }, actorUserId?: string): Promise<{ id?: string; error?: string }> {
  ...
  if (!mosque || mosque.type !== 'mosque') return { error: 'يلزم الالتحاق بمسجد' }
...
// التأهيل: أعلى N في الترتيب يصبحون «متأهلين» (التصفية النهائية)
export async function qualifyTop(db: Db, competitionId: string, topN: number) {
  const board = await leaderboard(db, competitionId)
  const top = board.slice(0, topN)
  for (const row of top) {
    await db.update(participants).set({ status: 'qualified' }).where(eq(participants.id, row.participantId)).run()
  }
  await db.update(competitions).set({ status: 'qualifying' }).where(eq(competitions.id, competitionId)).run()
  return top.map((r) => ({ participantId: r.participantId, personId: r.personId, total: r.total }))
}

// اختيار الفائز (صاحب الترتيب الأول) وإغلاق المسابقة
export async function selectWinner(db: Db, competitionId: string) {
  const board = await leaderboard(db, competitionId)
  if (!board.length) return { error: 'لا مشتركون' }
  const winner = board[0]
  await db.update(participants).set({ status: 'winner' }).where(eq(participants.id, winner.participantId)).run()
  await db.update(competitions).set({ status: 'closed' }).where(eq(competitions.id, competitionId)).run()
