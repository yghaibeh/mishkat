...
export async function addStudentEvaluation(db: Db, input: { enrollmentId: string; lessonSessionId: string; score?: number; note?: string; externalActivities?: string }) {
  const id = crypto.randomUUID()
  await db.insert(studentEvaluations).values({
    id, enrollmentId: input.enrollmentId, lessonSessionId: input.lessonSessionId,
    score: input.score ?? null, note: input.note ?? null, externalActivities: input.externalActivities ?? null, createdAt: Date.now(),
  }).run()
  return { id }
}
