...
export async function buildTracks(db: Db, personId: string, month: string): Promise<Track[]> {
  ...
  if (asg.some((a) => a.role === ROLES.ADMIN)) {
    const fixed = await rateForMonth(db, 'fixed_salary', month)
  ...
  if (amirMosques.length) {
    const pr = await rateForMonth(db, 'point_rate', month)
    let totalPoints = 0
...
export async function computeMonthlyEntitlement(db: Db, personId: string, month: string, actorUserId?: string) {
  ...
  // ق٣: الاعتمادُ قرارٌ متعمَّد — لا تمحوه إعادةُ الحساب (كان يرتدّ approved⇒proposed فيُفقد الاعتماد)
  if (prev?.status === 'approved') return { id: prev.id, status: 'approved', grossAmount: prev.grossAmount, tracks, note: 'معتمَد — لا يُعاد الحساب' }

  // ط٢: إزالةُ الترشيح القديم وإعادةُ بنائه في دفعةٍ ذرّيّةٍ واحدة (db.batch) —
  // كان حذفًا ثم إدراجًا منفصلَين، فعطبٌ بينهما يمحو مستحقَّ الشخص الماليّ بلا رجعة.
  const id = crypto.randomUUID()
  const now = Date.now()
  const stmts: unknown[] = []
  if (prev) {
    stmts.push(db.delete(entitlementTracks).where(eq(entitlementTracks.entitlementId, prev.id)))
    stmts.push(db.delete(monthlyEntitlements).where(eq(monthlyEntitlements.id, prev.id)))
  }
  stmts.push(db.insert(monthlyEntitlements).values({
    id, personId, month, grossAmount: gross, currency: 'USD', status: 'proposed', approvedBy: null, createdAt: now,
  }))
  for (const t of tracks) {
    stmts.push(db.insert(entitlementTracks).values({
...
export async function recordPayout(db: Db, entitlementId: string, paidAmount: number, recordedBy: string, reference?: string) {
  ...
  // ه٦: صرفُ الراتب يُرحَّل للدفتر (مدين الرواتب / دائن النقد) — غيرُ حرج
  try { const { postPayout } = await import('./ledgerPost'); await postPayout(db, { id, amount: paidAmount, createdBy: recordedBy }) }
  catch (e) { console.error('[ledger] postPayout failed:', (e as Error)?.message ?? e) }
