...
// ===== ر.١ تقرير الرولّ-أب القياديّ (شبكة/قسم/شهر) — عربيّ RTL للطباعة→PDF =====
type Rollup = {
  section: string; leafNoun: string; scopeName: string; month: string | null; generatedAt: number;
  totals: { mosques: number; enteredPct: number; avgPoints: number; done: number; below: number; struggling: number; circles: number };
  rows: Array<{ name: string; typeLabel: string; mosques: number; enteredPct: number; avgPoints: number; done: number; below: number; struggling: number; circles: number }>;
  financeTotal: number; financeMonth: string | null;
};

export function networkRollupHtml(r: Rollup): string {
  const sectionAr = r.section === "women" ? "قسم النساء" : "قسم الذكور";
  const body = r.rows.map((x) => `
      <tr>
        <td>${esc(x.name)}</td>
        <td>${esc(x.typeLabel)}</td>
        <td>${x.mosques}</td>
        <td>${x.enteredPct}%</td>
        <td>${x.avgPoints}</td>
        <td>${x.done}</td>
...
<title>تقرير قياديّ — ${esc(r.scopeName)} — ${esc(sectionAr)}</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  tfoot td { font-weight: 700; background: #f4f3ee; }
  .foot { margin-top: 24px; font-size: 12px; color: #888780; display: flex; justify-content: space-between; }
  @media print { body { padding: 0; } @page { size: A4 landscape; margin: 12mm; } .noprint { display: none; } }
...
<div class="sheet">
  ...
  <div class="head">
    <div>
      <h1>تقرير قياديّ — ${esc(r.scopeName)}</h1>
      <div class="sub">${esc(sectionAr)} · الشهر ${esc(r.month ?? "—")} · مِشكاة</div>
    </div>
    <span class="badge">${r.rows.length} وحدة</span>
  </div>

  <div class="cards">
    <div class="card"><div class="lab">${esc(r.leafNoun)}</div><div class="val">${t.mosques}</div></div>
    <div class="card"><div class="lab">نسبة الإدخال</div><div class="val ok">${t.enteredPct}%</div></div>
    <div class="card"><div class="lab">متوسط النقاط</div><div class="val">${t.avgPoints}<span style="font-size:13px;color:#888780"> / 70</span></div></div>
    <div class="card"><div class="lab">المتعثّرون</div><div class="val">${t.struggling}</div></div>
  </div>

  <h2>تفصيل الوحدات</h2>
  <table>
    <thead><tr><th>الوحدة</th><th>النوع</th><th>${esc(r.leafNoun)}</th><th>الإدخال</th><th>م. النقاط</th><th>مكتمل</th><th>دون الهدف</th><th>متعثّر</th><th>حلقات</th></tr></thead>
    <tbody>${body || '<tr><td colspan="9">لا وحدات</td></tr>'}</tbody>
    <tfoot><tr><td>الإجمالي</td><td>—</td><td>${t.mosques}</td><td>${t.enteredPct}%</td><td>${t.avgPoints}</td><td>${t.done}</td><td>${t.below}</td><td>${t.struggling}</td><td>${t.circles}</td></tr></tfoot>
  </table>

  <div class="foot">
    <span>وُلّد آليًّا من منصّة مِشكاة</span>
    <span>إجماليّة مستحقات الشهر (${esc(r.financeMonth ?? "—")}): $${r.financeTotal.toFixed(2)}</span>
  </div>
</div>
</body>
</html>`;
