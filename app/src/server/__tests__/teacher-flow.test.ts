...
describe("ملاحظات المشرف/الإدارة محميّة من المعلّم (العدسة ع٦ — تدقيق ٣٣ أ-٥)", () => {
  ...
  it("واجهة الحلقة لا تكذب: canManage بلا المدير (الإدخال للمعلم المالك/أمير المكان حصراً)", async () => {
    setUser(teacherA);
    const a = await ala.createMyHalaqaData({ name: "حلقة الصدق", curriculum: "baseera" });
    setUser(admin);
    const acc = await ala.halaqaAccessData(a.id!);
    expect(acc.canManage).toBe(false); // كان true فتُعرض له نماذجُ إدخالٍ يرفضها الخادم
    expect(acc.canSupervise).toBe(true); // اطلاعُه وإشرافُه باقيان
    setUser(teacherA);
    expect((await ala.halaqaAccessData(a.id!)).canManage).toBe(true);
  });
});
