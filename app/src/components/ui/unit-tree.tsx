...
export function UnitTree<L extends Leaf>({
  ...
  lazy = false, loadLeaves, counts, leafKind = "container",
}: {
  units: TreeUnit[];
  leaves: L[];
  ...
  // نوعُ الأوراق (قرار المالك ٢٠٢٦-٠٧-١٨ — التوسيع الافتراضي):
  //  "container" (حلقات/مساجد): الهيكليةُ كلُّها مفتوحةٌ افتراضاً حتى تظهر الأوراق.
  //  "person" (طلاب/مستفيدون): تُفتح الهيكلية وتبقى حاويةُ الأشخاص مطويةً (لا ٥٠ طالباً دفعةً).
  leafKind?: "container" | "person";
}) {
  ...
  // التوسيع الافتراضي (قرار المالك): «الكل» حتى مستوى الحاويات — الهيكليةُ مفتوحةٌ من أول نظرة؛
  // وفي الوضع الكسول أو أوراق الأشخاص تبقى الوحدةُ ذاتُ الأوراق المباشرة مطويةً (تُفتح بنقرة).
  const defaultOpen = useMemo(() => {
    const s = new Set<string>();
    for (const u of units) {
      const hasChildUnits = (childUnits.get(u.id) ?? []).length > 0;
      const directLeaves = lazy ? (counts?.[u.id] ?? 0) > 0 : (leavesByUnit.get(u.id) ?? []).length > 0;
      if ((lazy || leafKind === "person") && directLeaves && !hasChildUnits) continue;
      s.add(u.id);
    }
    return s;
  }, [units, childUnits, leavesByUnit, lazy, counts, leafKind]);
  const [openState, setOpen] = useState<Set<string> | null>(null); // null = الافتراضي المفتوح
