...
describe("الاعتمادُ الثنائيّ يعبر القاعدة — حمولةٌ مجمَّدةٌ تعود كما جُمِّدت (ق-٥٣)", () => {
  it("الفعلُ المعلَّق وحمولتُه وتاريخُها تعود من القاعدة بلا انحراف", async () => {
    ...
    await session(driver, MAIN, ({ ledger }) => {
      ...
      expect(ledger.audit().map((a) => a.action)).toEqual(["ledger.action.propose"])
    })

    const rows = await driver.all({
      sql: "SELECT unit_path, scope_exact FROM audit_log WHERE source = 'ledger'",
      params: [],
    })
  ...
  it("سجلُّ الشجرة يعود من القاعدة بترتيبه ونطاقه", async () => {
    ...
    await session(driver, MAIN, ({ org }) => {
      expect(org.audit.map((a) => a.targetId)).toEqual(["p-1", "p-2"])
      expect(org.audit[0]!.scopePath).toBe("/men/r1/m1/")
    })
    driver.close()
  })
})
...
describe("حوافُّ الطبقة — دفاعاتٌ تُختبر لا تُفترض", () => {
  ...
  it("ساعةُ الهجرة **تُحقن** — فلا زمنَ تشغيلٍ داخل الطبقة (TESTING_POLICY §٥)", async () => {
    ...
    expect(rows.map((r) => Number(r["applied_at"]))).toEqual([1_700_000_000_000])
    driver.close()
  })
    ...
    await driver.batch([
      {
        sql:
          ...
          " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        params: [
          MAIN, "/men/r1/m1/", "org", 1, 1_752_000_000_000, "u-admin", "legacy.action",
          null, null, "p-9", null, 1, null, null, null, null, null, null, null,
        ],
      },
    ])
    await session(driver, MAIN, ({ org }) => {
      expect(org.audit).toHaveLength(1)
      expect(org.audit[0]!.capability).toBe("")
      expect(org.audit[0]!.targetType).toBe("")
    })
    driver.close()
  })
})
...
describe("حارسُ انحراف العدّاد — القاعدةُ لا تُقرأ على عمياء", () => {
  ...
  it("دفترُ الهجرات يُقرأ بأسمائه — وقراءتُه على قاعدةٍ بكرٍ تُنشئه ولا ترمي", async () => {
    ...
    bare.close()
    const driver = await freshDb()
    expect(await appliedMigrations(driver)).toEqual(["0001_org_ledger_pilot.sql"])
    driver.close()
  })
})
