...
describe("حوافُّ مستودع اللجان — والسلبُ أكثرُ من الإيجاب", () => {
  ...
  it("**شكلُ توجيهٍ واحد** (الوصفة §٤-٠): كلُّ بيانات الوحدة بمسار وحدتها — ولا صفَّ جذرٍ واحد", async () => {
    const driver = await freshDb()
    await seedCommitteeSession(driver, MAIN)
    await committeeSession(driver, MAIN, (store) => {
      form(store, RELIEF)
    ...
    // **لا كتالوجَ ولا قاموسَ فئاتٍ في هذه الوحدة**: جداولُ بياناتها **تشغيليةٌ بأسرها**
    // ولا صفَّ جذرٍ في واحدٍ منها ⟵ لا جلسةَ تحتاج نطاقَ الجذر لتقرأ مرجعاً، فلا يفرض
    // الأضيقُ على الأوسع حملَه ⟵ **مستودعٌ واحدٌ هو الصواب** (لا فصلَ كتالوج/قيود).
    //
    // **والقائمةُ تُشتقّ من المستودع نفسِه ولا تُسرد** (CR-011/قب-٣٦): جدولٌ يُضاف غداً إلى
    // هذا المصنع يدخل الفحصَ **تلقائياً** — ولو سردتُ الأربعةَ لبقي كتالوجٌ جديدٌ بلا حارس.
    // ويُستثنى منها **مِرفقُ الطبقة** (`sequences`) و**مرآةُ الشجرة** (`committee_units`)،
    // وكلاهما مُبرهَنٌ أدناه على حدة لا مسكوتٌ عنه.
    const owned = persistentCommittee(freshCommitteeStore(MAIN))
      .tables.map((t) => (typeof t === "string" ? t : t.table))
      .filter((t) => t !== "sequences" && t !== "committee_units")
    expect(owned).toEqual(["committees", "committee_members", "committee_activities", "committee_meetings"])
    for (const t of owned) {
      const rows = (await rowsOf(driver, t)) as readonly Record<string, unknown>[]
      // **ويُوجب أن يكون الجدولُ ممارَساً**: جدولٌ يملكه المصنعُ ولا يكتب فيه هذا السيناريو
      // يُفشل الحارسَ بدل أن يمرّ فراغاً — فلا يخضرّ فحصٌ لم يفحص شيئاً.
      expect(`${t}:${rows.length > 0}`).toBe(`${t}:true`)
      expect(`${t}:${rows.every((r) => String(r["unit_path"]) !== "/")}`).toBe(`${t}:true`)
    }
    // و`committee_units` **مرآةُ شجرةٍ لا كتالوج**: تحمل ما تحمله الشجرةُ من مساراتٍ — ومنها
    // عقدةُ الجذر «الشبكة» نفسُها. وهذا **ليس بياناتٍ مرجعيةً تُقرأ بالجذر** بل عقدةٌ واحدة
    // تركب شرطَ `unit_path = '/'` في مُرشِّح النطاق؛ **ولا تُوسّع نطاقَ جلسةٍ ولا تجرّ صفّاً
    // تشغيلياً معها** — والدليلُ اختبارُ عزل النطاق أعلاه (جلسةُ بلال لا تحمّل لجنةَ خالد).
    const units = (await rowsOf(driver, "committee_units")) as readonly Record<string, unknown>[]
    expect(units.filter((r) => String(r["unit_path"]) === "/").map((r) => String(r["id"]))).toEqual(["root"])
    // والجذرُ لا يسكنه من مطالبي سواها إلا **مِرفقُ الطبقة** (`sequences`) الذي تتقاسمه المستودعاتُ.
    const seq = (await rowsOf(driver, "sequences")) as readonly Record<string, unknown>[]
    expect(seq.filter((r) => String(r["name"]) === "committees.seq").map((r) => String(r["unit_path"]))).toEqual(["/"])
    driver.close()
  })
