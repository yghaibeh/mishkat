...
describe("حوافُّ مستودع اللجان — والسلبُ أكثرُ من الإيجاب", () => {
  ...
  it("**شكلُ توجيهٍ واحد** (الوصفة §٤-٠): كلُّ بيانات الوحدة بمسار وحدتها — ولا صفَّ جذرٍ واحد", async () => {
    ...
    // **لا كتالوجَ ولا قاموسَ فئاتٍ في هذه الوحدة**: جداولُ البيانات الأربعةُ **تشغيليةٌ
    // بأسرها** ولا صفَّ جذرٍ في واحدٍ منها ⟵ لا جلسةَ تحتاج نطاقَ الجذر لتقرأ مرجعاً، فلا
    // يفرض الأضيقُ على الأوسع حملَه ⟵ **مستودعٌ واحدٌ هو الصواب** (لا فصلَ كتالوج/قيود).
    for (const t of ["committees", "committee_members", "committee_activities", "committee_meetings"] as const) {
