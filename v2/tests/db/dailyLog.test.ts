...
function record(
  ...
  opts: {
    ...
    attendees?: number
    actor?: string
    /** يومُ القيد — المفتاحُ الطبيعيّ `(وحدة، نشاط، يوم)`، فتنويعُه هو ما يُنتج قيوداً كثيرة. */
    date?: Date
  },
): DailyEntry {
...
/**
 * وحدةُ عملٍ محمَّلةٌ بـ**مصدرَيها** (§٤-٠ · CR-029) مع مقبضٍ على مصدر القيود — فما كان
 * يُقاس على مصدرٍ واحدٍ يُقاس عليه هو، **بلا تخفيفِ توكيدٍ واحد**.
 */
async function hydrated(
  ...
  uow: UnitOfWork
  store: DailyLogStore
  entries: ReturnType<typeof persistentDailyEntries>
}> {
  const stores = freshDailyLogStores(tenantId)
  const entries = persistentDailyEntries(stores.dailyLog)
  const uow = new UnitOfWork(driver, { tenantId, scopePath: "/" })
  uow.enlist(persistentDailyCatalog(stores.dailyLog))
  uow.enlist(entries)
  await uow.hydrate()
  return { uow, store: stores.dailyLog, entries }
}
...
async function statementsAfter(
  ...
  const { uow, store, entries } = await hydrated(driver, tenantId)
  fn(store)
...
describe("ق-٤١/ق-٤٥ — قيودٌ لا تُمحى: الاختفاءُ يُرمى ولا يُترجم `DELETE`", () => {
  it("ق-٤٥ — محوُ قيدٍ من الإسقاط **يُرمى**، ولا `DELETE` يُولَّد (النقطةُ مالٌ ق-٣٣)", async () => {
    ...
    const { uow, entries } = await hydrated(driver, MAIN)
    const forged = new Map(entries.project())
    forged.set("daily_entries", new Map())
    expect(() => uow.statementsFor(entries.name, forged)).toThrow(/daily_entries/)
    driver.close()
  })
...
describe("الذرّية — فشلٌ في منتصف عمليةٍ لا يترك نصفَ أثر", () => {
  ...
  it("وحدةُ عملٍ خليطةٌ تُرمى — لا يُقذف سجلٌّ ومستودعٌ بلا مخطط", async () => {
    ...
    uow.enlist(persistentDailyEntries(new DailyLogStore(MAIN)))
    uow.enlist({
...
describe("ميزانيةُ التحميل — لكلِّ مصدرٍ سقفُه المُسنَد إلى حمولته (G23 · §٤-٠ · CR-029)", () => {
  /**
   * **الحارسُ البنيويّ** — يُثبت **الإعلان**: مصدران لا واحد، ولا جدولَ يجمعهما.
   * (والبنيويُّ وحدَه لا يكفي — يليه السلوكيُّ أدناه: فخّ ٦-ب.)
   */
  it("**مصدران لا واحد**، ولكلٍّ سقفٌ موجبٌ مُعلَنٌ في مصنعه — والكتالوجُ أضيقُ من القيود", () => {
    const store = new DailyLogStore(MAIN)
    const catalog = persistentDailyCatalog(store)
    const entries = persistentDailyEntries(store)
    expect(`${catalog.name}:${catalog.rowBudget > 0}`).toBe("dailyLog.catalog:true")
    expect(`${entries.name}:${entries.rowBudget > 0}`).toBe("dailyLog.entries:true")
    // سقفُ الكتالوج **أصغرُ بمراتب**: مخطّطاتٌ وأنشطةٌ لا تنمو مع الميدان.
  ...
  it("**وتجاوزُه رميةٌ تُسمّي المصدرَ والجدولَ الأكبر** — لا «تجاوزٌ» مبهمة، وبزنادِ CR-026", async () => {
    const driver = await freshDb()
    await seedDailyLogSession(driver, MAIN)
    await dailyLogSession(driver, MAIN, (store) => {
    ...
    const uow = new UnitOfWork(driver, { tenantId: MAIN, scopePath: "/" })
    uow.enlist({ ...persistentDailyEntries(stores.dailyLog), rowBudget: 2 })
    await expect(uow.hydrate()).rejects.toThrow(/وحدةُ عمل «dailyLog\.entries»/)
    await expect(uow.hydrate()).rejects.toThrow(/daily_entries=/)
    await expect(uow.hydrate()).rejects.toThrow(/CR-026/)
    driver.close()
  })

  /**
   * **الحارسُ السلوكيّ** — يُثبت **الأثر** لا الإعلان (فخّ ٦-ب): قبل الفصل كانت جلسةُ
   * `activityCatalog.view` — ونطاقُها الجذرُ صراحةً — تجرّ `LIKE '/%'` ⟵ **كلَّ قيود الشبكة**.
   * وبعده **لا تلمسها أصلاً**.
   *
   * **والبذرةُ مقصودةُ الحجم**: ستّون قيداً مقابل سقفٍ مُصطنَعٍ **خمسين** — فلو عاد الكتالوجُ
   * يملك جدولاً تشغيلياً **لرمت الميزانيةُ هنا**. (درسُ `library`: حارسٌ ببذرةٍ أصغرَ من سقفه
   * **يمرّ في الحالتين**، فلا يكون حارساً بل زينة.)
   */
  it("**جلسةُ الكتالوج بالجذر لا تُحمّل قيداً واحداً ولا عددَ أُسرة** — وهذا هو الفصل", async () => {
    const driver = await freshDb()
    await seedDailyLogSession(driver, MAIN)
    await dailyLogSession(driver, MAIN, (store) => {
      // ستّون يوماً ⟵ ستّون قيداً: المفتاحُ الطبيعيّ `(وحدة، نشاط، يوم)` — لا مضاعفةَ في اليوم.
      ...
      setFamilyRoster(store, dailyLogContext("u-amir"), { unitId: KHALID, studentCount: 30 })
    })
    expect((await rowsOf(driver, "daily_entries")).length).toBe(60)

    // ١) بالمصنع كما يُشحن: الكتالوجُ يصل، **ولا صفَّ تشغيليٍّ واحد** يعبر إلى الذاكرة.
    const store = new DailyLogStore(MAIN)
    await dailyCatalogUnitOfWork(driver, store).hydrate()
    expect(store.schemes().length).toBeGreaterThan(0)
    expect(store.activities().length).toBeGreaterThan(0)
    expect(store.entries()).toEqual([])
    expect(store.rosters()).toEqual([])
    expect(store.units()).toEqual([])

    // ٢) وبسقفٍ مُصطنَعٍ **٥٠** — حتى **يقدر الحارسُ أن يحمرّ**: يسع الكتالوجَ (~٦) ولا يسع
    // الستّين قيداً. فلو عاد الكتالوجُ يملك جدولاً تشغيلياً لرمت الميزانيةُ هنا.
    const tight = new UnitOfWork(driver, { tenantId: MAIN, scopePath: "/" })
    tight.enlist({ ...persistentDailyCatalog(new DailyLogStore(MAIN)), rowBudget: 50 })
    await tight.hydrate()
    driver.close()
  })
})
...
describe("حوافُّ مستودع سجل اليوم — والسلبُ أكثرُ من الإيجاب", () => {
  it("تحميلٌ من لا شيء لا يرمي ولا يخترع — قاعدةٌ فارغةٌ مستودعٌ فارغٌ وعدّادٌ من الصفر", () => {
    const store = new DailyLogStore(MAIN)
    // **المصدرانِ كلاهما** يُحمَّلان من لا شيء — فلا يبقى فرعُ `rows.get(name) ?? new Map()` بلا قياس.
    persistentDailyCatalog(store).load(new Map())
    persistentDailyEntries(store).load(new Map())
    expect(store.entries()).toEqual([])
