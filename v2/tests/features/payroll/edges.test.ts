...
describe("قشرةُ القدرات — **الشخصيةُ لا تُحسب بنطاق** (§١.١)", () => {
  ...
  /**
   * **الضمانةُ الحقيقية — في المحرّك لا في القشرة** (اكتُشف بالكسر المُوجَّه، قب-٤٦ §١):
   * كان في `screens/caps.ts` سطرُ تخطٍّ للقدرات الشخصية **ميّتٌ مرّتين** (القدرةُ ليست في
   * القائمة، و`can()` يردّها أصلاً)، فكسرُه لم يُسقط اختباراً — **حارسٌ ديكور** (المادة ٠).
   * فحُذف، وهذا التوكيدُ هو ما حلّ محلَّه: **المحرّكُ نفسُه يردّ الشخصيةَ بنطاقٍ منطاق**
   * ⇒ فلا تتسرّب إلى قشرةٍ منطاقة **ولو أُدرجت في `SCREEN_SURFACE_CAPS` سهواً غداً**.
   * وهذه ضمانةٌ **بنيويةٌ لا انضباطية**، ولذلك تُختبر على **كل دورٍ حيّ** لا على واحد.
   */
  it("**والضمانةُ بنيويةٌ في المحرّك**: `can()` يردّ الشخصيةَ بنطاقٍ منطاقٍ لكل دورٍ حيّ", () => {
    const leaks: string[] = []
    for (const personId of ["u-admin", "u-finance", "u-amir", "u-teacher", "u-section-head"]) {
      const decision = can(canonicalActor(personId), "payroll.own", unitScope(KHALID_PATH), DECISION)
      if (decision.allowed) leaks.push(`${personId} نال «كشفَ راتبي» بنطاقٍ منطاق`)
      else expect(decision.reason, personId).toBe("DENIED_PERSONAL_NOT_OWNER")
    }
    expect(leaks, "**قدرةٌ شخصيةٌ تُمنح بالنطاق** — نقضٌ لق-٢٧/ق-٢٩").toEqual([])
  })
})
