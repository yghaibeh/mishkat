...
describe("ق-٥ + ب-٣٠أ — لا اعتمادَ لمسودة، والتجاوزُ المنسوخُ لا أثرَ له", () => {
  ...
  it("**وتفعيلُ `approval.draft_bypass_enabled` لا يغيّر شيئاً** — الإعدادُ الميت لا يُعطّل حارساً (CR-008)", () => {
    const s = store()
    const request = submitted(s)
    retractSubmission(s, approvalContext("u-amir"), { requestId: request.id })

    const bypass = approvalContext("u-square", {
      settings: [
        {
    ...
    const decided = approveRequest(s, bypass, { requestId: request.id })
    expect(!decided.ok && decided.error.code).toBe("NOT_SUBMITTED")
  })
