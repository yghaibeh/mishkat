...
// **ترحيلُ واجهةٍ لا تعديلُ توكيد** (CR-020): قراءةُ المستودع صارت تسأل عن **الفترة** لأنها
// ضلعٌ في مفتاحه — والقيمُ المتوقَّعةُ هنا كما هي حرفاً بحرف.
...
describe("ق-٩٠ — الجلسةُ اليومية: حضورٌ رباعيّ، ومفتاحُها (حلقة × يوم)", () => {
  ...
  it("**وإعادةُ الإرسال آمنة** (upsert): اليومُ نفسُه يُستبدَل ولا تُولَد جلسةٌ ثانية", () => {
    ...
    expect(
      world.log.getSession(world.circleId, "2026-07-22", WHOLE_DAY_PERIOD_ID)?.rows[0]?.attendance,
    ).toBe("present")
  })
