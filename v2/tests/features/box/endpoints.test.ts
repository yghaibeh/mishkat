...
describe("الإقرارُ نطاقُه **شخصيّ** مشتقٌّ من التسليم المخزَّن (§١.١ + خ-٧)", () => {
  ...
  it("**والمديرُ يُرفض بـ`DENIED_PERSONAL_NOT_IN_ROLE`** — لا يقرّ عن الأمين ولو ملك كلَّ شيء", async () => {
    const { ep, handoverId } = await seedHandover()
    const r = await ep.acknowledge.invoke({ handoverId }, canonicalActor("u-admin"), WRITE)
    expect(r.ok).toBe(false)
    if (r.ok) return
    // **CR-012/قب-٣٨**: حزمةُ المدير لا تحمل `box.handover.acknowledge` أصلاً (المصفوفة)،
    // فالمنعُ يقع عند الشرط الأول — والقرارُ (منعٌ) هو هو، والتشخيصُ أدقّ.
