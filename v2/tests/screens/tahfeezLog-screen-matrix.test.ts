...
import { makeTahfeezLogEndpoints } from "../../src/features/tahfeezLog/server/endpoints.js"
import { circleModelFrom } from "../../src/features/tahfeezLog/services/circlesPort.js"
import {
  computeTahfeezLogCaps,
  teachesAnyCircle,
} from "../../src/features/tahfeezLog/screens/caps.js"
import {
  logDayScreenNodes,
  myLogScreenNodes,
  notesScreenNodes,
  EMPTY_TAHFEEZ_LOG_SNAPSHOT,
  LOG_DAY_CONTRACT,
  MY_LOG_CONTRACT,
  NOTES_CONTRACT,
} from "../../src/features/tahfeezLog/screens/screens.js"
...
import {
  ...
  WRITE,
  type World,
} from "../features/tahfeezLog/_seed.js"
...
type Ep = ReturnType<typeof makeTahfeezLogEndpoints>

function endpointsFor(world: World): Ep {
  return makeTahfeezLogEndpoints({
    store: world.log,
...
function capsFor(world: World, actor: Actor): ReadonlySet<CapId> {
  ...
  return computeTahfeezLogCaps(actor, KHALID_PATH, DECISION, teachesAnyCircle(port, actor.personId))
}
...
function reaches(caps: ReadonlySet<CapId>, cap: CapId): boolean {
  const seen = new Set([
    ...visibleCaps(logDayScreenNodes(caps, EMPTY_TAHFEEZ_LOG_SNAPSHOT)),
    ...visibleCaps(notesScreenNodes(caps, EMPTY_TAHFEEZ_LOG_SNAPSHOT)),
    ...visibleCaps(myLogScreenNodes(caps, EMPTY_TAHFEEZ_LOG_SNAPSHOT)),
  ])
  return seen.has(cap)
}
...
describe("عدساتُ الشاشات وعقودُها (G20 — لا دور يرى ما خارج عدسته)", () => {
  it("**فراغُ المطّلع مُشخِّصٌ لمن لا يملك بابَ الشاشة** — لا شاشةٌ بيضاء (ق-١١٢)", () => {
    ...
    for (const fixture of ROLE_FIXTURES) {
      ...
      const view = logDayScreenNodes(caps, EMPTY_TAHFEEZ_LOG_SNAPSHOT)
  ...
  it("**والمعلّمُ لا يجد مِقبضَ تحريرِ ملاحظةِ المشرف على شاشته** (ب-٣٥أ — الطبقةُ الأولى)", () => {
    ...
    const view = myLogScreenNodes(caps, EMPTY_TAHFEEZ_LOG_SNAPSHOT)
