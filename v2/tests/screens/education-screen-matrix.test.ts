...
function endpointsOf(w: EduWorld): Ep {
  return makeEducationEndpoints(w.education, educationPorts(w), SETTINGS, () => false)
}
...
describe("مصفوفةُ شاشات «على بصيرة» — حضورٌ بعدسة الدور، وغيابٌ مقرونٌ برفض الخادم", () => {
    ...
    for (const f of ROLE_FIXTURES) {
      for (const a of AFFORDANCES) {
        ...
        const caps = computeEducationCaps(
          ...
          KHALID_PATH,
          DECISION,
          lessonsOfTeacher(w.education, actor.personId).length > 0 || actor.personId === "u-teacher",
        )
  ...
  it("**والمواطنُ القانونية معلنةٌ بلا ازدواج**: الدرسُ والتقدّمُ والمنهاج، و«دروسي» بلا موطن", () => {
    expect([...CIRCLE_LESSONS_CONTRACT.canonicalHome]).toEqual(["lesson", "curriculumProgress"])
