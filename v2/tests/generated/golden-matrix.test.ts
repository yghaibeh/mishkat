...
describe("المصفوفة الذهبية دور×قدرة — كل خلية مُختبَرة", () => {
  for (const c of report.cases) {
    ...
    it(title, () => {
      ...
      // **القدرة الشخصية تُفحص بشرطَيها** (CR-012/قب-٣٨): حزمةُ الدور **و** ملكيةُ الكيان.
      // فصاحبُ الكيان هو الفاعلُ نفسُه إلا في الحالة التي يُقصد فيها اختبارُ الشرط الثاني.
      const ownEntity = c.variant !== "personal_holds_foreign_entity" && c.variant !== "override_deny"
      const scope =
        meta.type === "personal"
          ? c.expectAllowed || ownEntity
      ...
      // ولكلّ صنفٍ شخصيٍّ **سببُه المميِّز**: التشخيصُ جزءٌ من العقد (§٤.١) — ولولا هذا
      // التوكيد لَمرّت الحالتان بسببٍ واحدٍ فعادت الخلايا مُختبَرةً بغير معناها.
      if (c.variant === "personal_lacks_own_entity") {
        expect(decision.reason, title).toBe("DENIED_PERSONAL_NOT_IN_ROLE")
      }
      if (c.variant === "personal_holds_foreign_entity") {
        expect(decision.reason, title).toBe("DENIED_PERSONAL_NOT_OWNER")
      }
    })
  }
})
