 ...
 * (before/after)» — فـ«من كان يحوزها» لا يضيع ولو تبدّل عشراً.
 *
 * > ### ✅ **إلزاميّان قابلان للفراغ** — CR-028 مُنفَّذةً في T26-ج
 * > كانا **اختياريّين** في T26-ب-١ بثمنٍ معلن: إلزامُهما يومَها كان يوجب تحريرَ `box`
 * > و`payroll` **وهما محظورتان نصّاً** على تلك المهمة. فوُسِّع العقدُ بأقلّ ما يُمكن، ورُفعت
 * > المسوّدةُ، ونُفِّذ الإلزامُ **في مرورٍ واحدٍ بعد الموجة** كما قرّر مديرُ البرنامج.
 * >
 * > **ولماذا لم يبقيا اختياريَّين؟** لأن `before?` **لا يفرّق** بين **«لا لقطةَ تنطبق هنا»**
 * > و**«نسيَها الكاتب»** — وهما حالتان علاجُهما متعاكس. وهو بعينه ما عالجه `shape` بالاتحاد
 * > المميَّز (T22) و`TrackOutcome` في الرواتب (T23): **الغيابُ يُسمّى ولا يُترك فراغاً**.
 * > **والمجالُ هنا التدقيق** — الجدولُ الذي المادة ٤/٨ تقول عنه «لا يُمحى»: **وقيدٌ ناقصٌ
 * > صامتاً أسوأُ من قيدٍ غائبٍ صراحةً؛ الغائبُ يُبحَث عنه، والناقصُ يُصدَّق.**
 * >
 * > **والحارسُ بُني مع الإلزام لا بعده** (نظيرُ `capability`): نداءٌ بلا تصريحٍ **لا يُترجم**
 * > (خطأُ نوعٍ في G1)، ومسحٌ بنيويٌّ في `tests/db/audit-payload.test.ts` يشتقّ المُستدعين من
 * > الشجرة ويُثبت **صفرَ متخلّف** — فلا يُترك ذلك لانضباط كاتب.
 */
export type AuditPayload = {
  /**
   * لقطةُ الحال **قبل** الفعل. و`null` **قرارٌ صريحٌ يقول «لا لقطةَ تنطبق»** — لا إغفال:
   * إنشاءٌ لا حالَ له قبله، أو فعلٌ هدفُه ليس كياناً مخزَّناً أصلاً.
   */
...
export type AuditAppend = AuditPayload & {
 ...
 * و`AuditPayload` جزءٌ من `AuditAppend` نفسِه بعد CR-028، فلا تطبيعَ بينهما ولا فرقَ يُخفى.
 */
...
export class AuditJournal {
  ...
  append(entry: AuditAppend): AuditEntryRecord {
    ...
    const record: AuditEntryRecord = Object.freeze({
      ...entry,
      // **ولا تطبيعَ لـ«قبل/بعد» هنا بعد CR-028**: صارا إلزاميَّين في `AuditAppend`، فما
      // يصل **مُصرَّحٌ به أصلاً** — و`?? null` كانت تُغطّي إغفالاً لم يعد ممكناً أن يقع.
