...
/**
 * **CR-٠٢٠ — فترةُ الجلسة بياناتٌ مرجعية** (ق-٨٩: قائمةٌ محصورة لا كتابةٌ حرّة، ونظيرُ ب-٢٨):
 * صباحاً ومساءً **صفّان** لا اسمان في الكود — فشبكةٌ تعقد ثالثةً تُضيف صفّاً بلا سطر.
 * و`ordinal` **ترتيبُ عرضٍ لا دلالةُ عمل**: لا يفرّع عليه منطقٌ ولا يدخل في مفتاح.
 */
export type SessionPeriodRef = {
  readonly tenantId: string
  readonly id: string
  readonly ar: string
 ...
 * الطبيعيّ `(circleId, dayKey, periodId)` بعد CR-020، فإعادةُ الإرسال **تستبدل أسطرَ الفترة**
 * ولا تُنشئ جلسةً ثانيةً لها (ق-٩٠ «upsert»).
 *
 * **والمفتاحُ اتّسع ولم ينشطر الكيان** (CR-016 محفوظٌ بحرفه): الفترةُ **مميِّزٌ ثالثٌ** كالتاريخ
 * سواءً — لا حقلَ حالةٍ ولا نوعَ كيانٍ ثانٍ. وجلستان **بنفس الفترة** في اليوم ⇒ **صفٌّ واحدٌ**
 * لا صفّان (قرارُ المالك: لا يُفتح باب الازدواج الذي منعه المفتاح أصلاً).
 *
...
export type DaySession = {
  ...
  /**
   * **CR-٠٢٠** — فترةُ اليوم من القائمة المحصورة (`services/periods.ts`). و**لا تكون فارغةً
   * أبداً**: شبكةٌ لم تُقسّم يومَها فترتُها `day` («اليومُ كلُّه») — فالعمودُ `NOT NULL` بحقّه،
   * ولا يحتاج المفتاحُ الطبيعيُّ أن يحرس فراغاً.
   */
...
export type DayLogErrorCode =
  ...
  | "UNKNOWN_MUSHAF"
  | "PAGE_OUT_OF_RANGE"
  /** CR-٠٢٠ — فترةٌ ليست في القائمة المحصورة (ق-٨٩). */
  | "UNKNOWN_PERIOD"
  /** CR-٠٢٠ — شبكةٌ أعلنت فتراتٍ ولم تُقل الجلسةُ فترتَها: **رفضٌ لا اختيارٌ صامت**. */
  | "PERIOD_REQUIRED"
