...
export function postEventSafely(
  ...
  try {
    store.audit.append({
      ...
      reason: code,
      // **«لا لقطةَ تنطبق» تُقال ولا يُسكَت عنها** (CR-028): الترحيلُ **لم يقع**، وهدفُ هذا
      // القيد **مفتاحُ ترحيلٍ لا كيانٌ مخزَّن** — فليس ثمّةَ حالٌ قبلَه ولا بعدَه أصلاً.
      // وهو **الفراغُ الوحيدُ في الطرفين** في الشجرة كلِّها، ونظيرُه في `unitPath` قائمٌ
      // (`AUDIT_ACTIONS_WITHOUT_SCOPE`): **الاستثناءُ المُعلَنُ ليس ثغرة، والصمتُ هو الثغرة.**
      before: null,
      after: null,
    })
  } catch {
