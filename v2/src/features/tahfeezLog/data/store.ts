...
export class TahfeezLogStore {
  ...
  upsertSession(session: DaySession): DaySession {
    ...
    this.sessionMap.set(TahfeezLogStore.key(session.circleId, session.dayKey), sealed)
    return sealed
  }

  getSession(circleId: string, dayKey: string): DaySession | null {
    return this.sessionMap.get(TahfeezLogStore.key(circleId, dayKey)) ?? null
  }
