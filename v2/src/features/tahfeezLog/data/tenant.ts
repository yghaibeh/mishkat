...
import { TahfeezLogStore } from "./store.js"

export class TahfeezLogTenantRegistry {
  private readonly stores = new Map<string, TahfeezLogStore>()

  storeFor(tenantId: string): TahfeezLogStore {
    const existing = this.stores.get(tenantId)
    if (existing !== undefined) return existing
    const created = new TahfeezLogStore(tenantId)
