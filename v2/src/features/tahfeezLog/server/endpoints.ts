...
import type { TahfeezLogContext } from "../services/context.js"
import {
  recordSession,
...
export type TahfeezLogDeps = {
  readonly store: TahfeezLogStore
...
export function makeTahfeezLogEndpoints(deps: TahfeezLogDeps) {
  const { store, circles } = deps

  const contextOf = (actor: Actor, request: DecisionContext): TahfeezLogContext => ({
    now: request.now,
